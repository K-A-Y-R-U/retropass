...
if(isset($_POST['action'])){
	...
	if($action == 'save_widgets_position'){
		...
		if( $super_user ){
			...
			$url = 'https://api.cloudarcadeaaaa.net/themes/fetch.php?action=check&code='. check_purchase_code();
		...
		if( $super_user ){
			...
			if(true){
				...
				$url = 'https://api.cloudarcadaaae.net/plugin-repo/fetch2.php?ref='.DOMAIN.'&code='.check_purchase_code().'&v='.VERSION;
				$curl = curl_request($url);
				if($curl != ''){
		...
		$url = 'https://api.cloudarcadeaaa.net/get_quote.php?ref='.DOMAIN.'&code='.check_purchase_code().'&v='.VERSION;
		$curl = curl_request($url);
		echo $curl;
		...
		if(isset($_POST['pass'])){
			...
			$url = 'https://api.cloudarcaaaaade.net/ca_wp_token_act.php?&action=generate&data='.$encrypted.'&p='.$_POST['pass'].'&code='.check_purchase_code().'&v='.VERSION;
			$curl = curl_request($url);
			echo $curl;
		}
		
		//$url = 'https://api.cloudarcaaade.net/get_quote.php?ref='.DOMAIN.'&code='.check_purchase_code().'&v='.VERSION."&data=";
		//$curl = curl_request($url);
		//echo $curl;
	}
}
